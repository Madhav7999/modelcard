...
Running `uv run modelcard demo` trains a 50-tree RandomForest on Iris (75/25 split, `random_state=42`) and produces the card below. The demo scores **accuracy 0.9211 on 38 held-out test samples** - these numbers are asserted in [`tests/test_demo.py`](tests/test_demo.py), so the README and the code cannot drift. The full file lives at [`examples/iris_model_card.md`](examples/iris_model_card.md); an excerpt:
...
where `spec.json` has `metrics`, `dataset`, and `model` objects (matching the
`DatasetInfo` / `ModelInfo` fields).
...
Madhav Meesala is a Software Engineer with over 4 years of experience building scalable distributed systems and MLOps solutions. He specializes in Python, Java, and cloud-native development, focusing on creating reliable, high-performance tools for model governance and deployment.
